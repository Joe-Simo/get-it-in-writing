...
test("landing page tells the product story and remains accessible", async ({
  ...
  await expect(
    page.getByRole("heading", { level: 1, name: /signal garden/i }),
  ).toBeVisible();
  await expect(page.getByText("Decision research, with proof")).toBeVisible();
  await expect(
    page.getByRole("button", {
      name: /claim: verify financial capacity or access to financing/i,
    }),
...
test("evidence selection and static mode are operable", async ({ page }) => {
  ...
  const source = page.getByRole("button", {
    name: /source: construction industry/i,
  });
  await source.click();
  await expect(source).toHaveAttribute("aria-pressed", "true");
  await expect(
    page.getByRole("heading", { level: 2, name: /construction industry/i }),
  ).toBeVisible();
...
test("the real public decision leads with a complete brief and sponsor proof", async ({
  ...
  await page.goto("/garden/what-must-a-small-construction-firm--52a65131", {
    waitUntil: "domcontentloaded",
  });
  await expect(
    page.getByRole("heading", { level: 1, name: /small construction firm/i }),
  ).toBeVisible();
  await expect(
  ...
  await expect(
    page.getByText("1 brief delivery", { exact: true }),
  ).toBeVisible();
  await expect(
    page.getByText("1 verified reply", { exact: true }),
  ).toBeVisible();
  await expect(
