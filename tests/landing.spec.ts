...
test("the landing page makes the decision-protection outcome immediately clear", async ({ page }) => {
  ...
  await expect(page.getByText(/nothing is sent without your approval/i)).toBeVisible();
...
test("the public story describes the customer boundary rather than the stack", async ({ page }) => {
  await page.goto("/", { waitUntil: "domcontentloaded" });
  ...
  expect(visibleText).not.toMatch(/Firecrawl|AgentMail|OpenAI|Convex database|WebGPU/i);
  expect(visibleText).not.toMatch(/AI-powered|revolutionary|seamless|supercharge/i);
});

test("the primary action preserves the draft and reaches private sign in", async ({ page }) => {
  await page.goto("/", { waitUntil: "domcontentloaded" });
...
test("the decision intake remains usable at the mobile viewport", async ({ page }, testInfo) => {
