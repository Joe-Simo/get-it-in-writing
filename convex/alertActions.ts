  ...
  handler: async (ctx, args) => {
    ...
    try {
      ...
      const html = isTest
        ...
        : `<main style="font-family:system-ui;max-width:620px;margin:auto;color:#111"><p style="font-size:12px;text-transform:uppercase;letter-spacing:.12em;color:#a83222">Lead-form check failed</p><h1>${escapeHtml(brokenStep)}</h1><p>${escapeHtml(context.detail ?? "The check did not complete.")}</p><p><a href="${escapeHtml(checkUrl)}">Open the check details and rerun it</a></p><p style="color:#666">This is an automated check, not a customer lead.</p><p style="color:#666;font-size:12px">Delivery reference: ${escapeHtml(context.token)}</p></main>`;
