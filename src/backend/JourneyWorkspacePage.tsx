...
function JourneyControlRoom({ detail, journeys }: { detail: JourneyDetail; journeys: JourneyList }) {
  ...
  const displayStatus = detail.journey.status === "needs_review" ? "Needs review" : detail.journey.enabled ? detail.journey.status : "Paused";
  const statusTone = detail.journey.status === "incident" ? "border-[#d54f3d]/50 bg-[#d54f3d]/10 text-[#a83222]" : detail.journey.status === "healthy" ? "border-[#4f7134]/40 bg-[#4f7134]/8 text-[#385724]" : detail.journey.status === "needs_review" ? "border-[#a96b18]/45 bg-[#e9a23b]/10 text-[#754607]" : "border-black/25 bg-transparent";
  const run = () => {
      ...
      <main className="grid min-h-[calc(100vh-72px)] lg:grid-cols-[260px_minmax(0,1fr)_340px]">
        <aside className="hidden border-r border-black/20 p-4 lg:block">
          ...
          <nav className="mt-3 space-y-1" aria-label="Lead-form checks">
            {journeys.map((journey) => <Link key={journey._id} to={`/app/journeys/${journey._id}`} className={journey._id === detail.journey._id ? "block border border-black/20 bg-[#ded9ce] p-3" : "block border border-transparent p-3 hover:border-black/15"}><div className="flex items-center justify-between gap-3"><p className="text-sm font-semibold leading-tight">{journey.name}</p><span className={journey.status === "incident" ? "size-2 shrink-0 rounded-full bg-[#d54f3d]" : journey.status === "healthy" ? "size-2 shrink-0 rounded-full bg-[#4f7134]" : journey.status === "needs_review" ? "size-2 shrink-0 rounded-full bg-[#a96b18]" : "size-2 shrink-0 rounded-full bg-black/25"} /></div><p className="mt-2 text-[11px] capitalize text-black/45">{journey.status === "needs_review" ? "needs review" : journey.enabled ? journey.status : "paused"}</p></Link>)}
          </nav>
        </aside>
        <section className="min-w-0 border-r border-black/20">
          <div className="border-b border-black/20 p-6 md:p-9">
            <div className="flex flex-wrap items-center gap-2"><Badge variant="outline" className={statusTone}>{displayStatus}</Badge><Badge variant="outline" className="border-black/20 bg-transparent capitalize">{detail.journey.kind.replace("_", " ")}</Badge><span className="text-xs text-black/42">Checks every day</span></div>
          ...
          <div className="grid gap-px border-b border-black/20 bg-black/20 sm:grid-cols-3">
            ...
            <SummaryMetric label="Confirmation window" value={detail.steps.some((step) => step.kind === "confirmation") ? `${detail.journey.expectedReplyMinutes} min` : "On-page result"} />
          </div>
...
function CheckpointIcon({ kind, status }: { kind: "website" | "form" | "confirmation" | "human_reply"; status: string }) {
  ...
  if (status === "blocked") return <span className="grid size-8 place-items-center rounded-full bg-[#a96b18] text-white"><ShieldCheck className="size-4" /></span>;
...
function StatusIcon({ status }: { status: string }) { if (status === "healthy") return <CheckCircle2 className="size-4 text-[#4f7134]" />; if (status === "incident" || status === "error") return <CircleAlert className="size-4 text-[#d54f3d]" />; if (status === "blocked") return <ShieldCheck className="size-4 text-[#a96b18]" />; return <Clock3 className="size-4 text-black/45" />; }
