...
function JourneyControlRoom({ detail, journeys }: { detail: JourneyDetail; journeys: JourneyList }) {
  ...
  const run = () => {
    ...
    void runNow({ journeyId: detail.journey._id })
      .then(() => setMessage("The authorized customer journey is running now."))
      .catch((reason: unknown) => setError(reason instanceof Error ? reason.message : "The journey could not start"))
    ...
    <div className="min-h-screen bg-[#f3efe6] text-[#111612]">
      <header className="flex h-[72px] items-center justify-between border-b border-black/20 px-4 md:px-8">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button size="sm" className="rounded-full bg-[#c8ff53] text-[#111612] hover:bg-[#d8ff80]" disabled={!detail.journey.enabled || pending !== null || Boolean(activeRun)} onClick={run}>{pending === "run" ? <LoaderCircle className="animate-spin" /> : <Play />} Run now</Button>
        </div>
      </header>
      ...
      <main className="grid min-h-[calc(100vh-72px)] lg:grid-cols-[260px_minmax(0,1fr)_340px]">
        <aside className="hidden border-r border-black/20 p-4 lg:block">
          <div className="flex items-center justify-between px-2 py-3"><p className="eyebrow">Journeys</p><span className="font-mono text-xs text-black/40">{journeys.length}</span></div>
          <nav className="mt-3 space-y-1" aria-label="Customer journeys">
        ...
        <section className="min-w-0 border-r border-black/20">
          ...
          <div className="p-6 md:p-9">
            <div className="flex items-end justify-between"><div><p className="eyebrow">Current run</p><h2 className="mt-3 text-3xl font-semibold tracking-[-.045em]">Every customer handoff.</h2></div>{latestRun && <span className="text-xs capitalize text-black/45">{latestRun.status} · {formatTime(latestRun.startedAt)}</span>}</div>
            {latestRun ? <RunTimeline run={latestRun} /> : <JourneyPlan steps={detail.steps} />}
            {detail.runs.length > 1 && <div className="mt-12"><p className="eyebrow">Run history</p><div className="mt-4 divide-y divide-black/12 border-y border-black/15">{detail.runs.slice(1, 8).map((runItem) => <div key={runItem._id} className="flex items-center justify-between py-4 text-sm"><div className="flex items-center gap-3"><StatusIcon status={runItem.status} /><span className="capitalize">{runItem.status}</span><span className="text-black/42">{runItem.summary ?? "Run recorded"}</span></div><time className="text-xs text-black/42">{formatTime(runItem.startedAt)}</time></div>)}</div></div>}
          </div>
        </section>
        <aside className="p-5 md:p-7">
          <div className="flex items-center justify-between"><p className="eyebrow">Incident desk</p><Badge variant="outline" className="border-black/20 bg-transparent">{detail.incidents.filter((incident) => incident.status === "open").length} open</Badge></div>
          <div className="mt-5 space-y-3">{detail.incidents.filter((incident) => incident.status === "open").map((incident) => <IncidentCard key={incident._id} incident={incident} />)}{detail.incidents.every((incident) => incident.status !== "open") && <div className="border border-black/15 p-5"><CheckCircle2 className="size-5 text-[#4f7134]" /><h2 className="mt-4 text-lg font-semibold">No customer is waiting.</h2><p className="mt-2 text-sm leading-relaxed text-black/50">A failed page, form, confirmation, or reply promise will open here with evidence.</p></div>}</div>
          <div className="mt-8 border-t border-black/15 pt-6"><p className="eyebrow">Journey definition</p><div className="mt-4 space-y-3">{detail.steps.map((step) => <div key={step._id} className="flex items-start gap-3"><span className="mt-1 grid size-6 shrink-0 place-items-center rounded-full border border-black/20 font-mono text-[9px]">{step.order + 1}</span><div><p className="text-sm font-semibold">{step.label}</p><p className="mt-1 text-xs leading-relaxed text-black/45">{step.instruction}</p></div></div>)}</div></div>
        </aside>
      </main>
      <Dialog open={activateOpen} onOpenChange={setActivateOpen}>
        <DialogContent className="bg-[#f3efe6] sm:max-w-lg"><DialogHeader><DialogTitle className="text-3xl tracking-[-.045em]">Authorize this real customer check.</DialogTitle><DialogDescription>Signal Garden will submit the reviewed public form using a clearly labeled QA identity. It will never enter payment, login, sensitive personal data, upload files, bypass a captcha, or submit more than once per run.</DialogDescription></DialogHeader><div className="border border-black/15 p-4"><p className="text-sm font-semibold">{detail.journey.name}</p><p className="mt-1 text-xs text-black/50">{detail.journey.startUrl}</p></div><label className="flex items-start justify-between gap-5 text-sm leading-relaxed"><span>I own this journey or have permission to test it, and I approve the exact public form check described above.</span><Switch checked={authorized} onCheckedChange={setAuthorized} className="mt-1" /></label><Button disabled={!authorized || pending !== null} className="h-12 w-full rounded-full" onClick={() => { setPending("activate"); setError(""); void activate({ journeyId: detail.journey._id, authorizedPublicFormTesting: authorized }).then(() => { setActivateOpen(false); setMessage("Journey activated. No run starts until the schedule or Run now triggers it."); }).catch((reason: unknown) => setError(reason instanceof Error ? reason.message : "The journey could not be activated")).finally(() => setPending(null)); }}>{pending === "activate" ? <LoaderCircle className="animate-spin" /> : <ShieldCheck />} Authorize and activate</Button></DialogContent>
      </Dialog>
    </div>
  );
}
