...
export default function DecisionPage() {
  ...
  const openChanges = detail.sourceChanges.filter(
  ...
  return (
    <div className="decision-page">
      <Link to="/app" className="back-link">
        <ArrowLeft /> Promise Wallet
      </Link>
      <header className="decision-header">
        <div>
          <div className="flex flex-wrap items-center gap-2">
            <StatusStamp status={detail.decision.status} />
            <span className="category-chip">{detail.decision.category}</span>
          </div>
          <h1>{detail.decision.title}</h1>
          <a
            href={detail.decision.sourceUrl}
            target="_blank"
            rel="noreferrer"
            className="source-host"
          >
            <Link2 /> {detail.decision.sourceHost} <ArrowUpRight />
          </a>
        </div>
        <div className="decision-private">
          <LockKeyhole />
          <span>
            <strong>Private case</strong>
            <small>Only you can see this decision.</small>
          </span>
        </div>
      </header>
      ...
      {openChanges.map((change) => (
        <SourceChangeAlert
          key={change._id}
          change={change}
          pending={pending === change._id}
          onAcknowledge={() => void acknowledge(change._id)}
        />
      ))}
      ...
      <section className="exact-requirement ticket-shell">
        <div className="ticket-number">
          <span>
            {String(detail.requirements.length || 1).padStart(2, "0")}
          </span>
              ...
              <li key={requirement._id}>
                ...
                <div>
                  ...
                  <p>
                    ...
                    {requirement.scope && <small>{requirement.scope}</small>}
                  </p>
                </div>
              </li>
            ))}
        ...
        {detail.decision.context && (
          <p>
            <strong>Context:</strong> {detail.decision.context}
          </p>
        )}
      </section>
      ...
      <div className="decision-grid">
        <section className="reliance-map-panel">
          <div className="section-heading">
            <div>
              <p className="ink-label">Reliance map</p>
              <h2>What you can safely rely on</h2>
            </div>
            <span>
              {detail.assessments.length || (researchActive ? "…" : 0)}
            </span>
          </div>
          {researchActive && detail.assessments.length === 0 ? (
            <ResearchInProgress
              status={detail.decision.status}
              sourceHost={detail.decision.sourceHost}
              progress={progress}
            />
          ) : detail.assessments.length === 0 ? (
            <p className="panel-empty">No reliance map is available yet.</p>
          ) : (
            <div className="assessment-list">
              {detail.assessments.map((assessment) => (
                <Assessment
                  ...
                  evidence={evidenceByAssessment.get(assessment._id) ?? []}
                />
              ))}
            </div>
          )}
          {detail.sources.length > 0 && (
            <SourceRegister sources={detail.sources} />
          )}
        </section>
        ...
        <aside className="case-side">
          {request ? (
            ...
            />
          ) : terminalStatuses.has(detail.decision.status) ? null : (
            <ContactBoundary />
          )}
      ...
      {detail.proofCard && (
        <ProofCard
          card={detail.proofCard}
          items={detail.proofItems}
          followUpAlreadyPrepared={detail.requests.some(
      ...
      <section className="case-delete" aria-live="polite">
        <div>
          ...
          <p>
            Removes its requirements, sources, messages, and Proof Card from
            this wallet. Previously sent email cannot be recalled.
          </p>
        </div>
        {deleteArmed ? (
          ...
          <Button variant="outline" onClick={() => setDeleteArmed(true)}>
            <Trash2 /> Delete case
          </Button>
        )}
      </section>
...
function StatusRail({ status }: { status: string }) {
  ...
  const index = ["draft", "scoping", "researching"].includes(status)
    ? 0
    : status === "analyzing"
      ? 1
      : [
        ...
        ? 2
        : ["reply_received", "interpreting_reply"].includes(status)
          ? 3
          : 4;
  return (
    <ol className="status-rail" aria-label="Decision progress">
      {points.map((point, pointIndex) => (
        <li
          key={point.key}
          className={
            pointIndex < index ? "done" : pointIndex === index ? "current" : ""
          }
        >
          <span>{pointIndex < index ? <Check /> : pointIndex + 1}</span>
          {point.label}
        </li>
      ))}
    </ol>
...
function ResearchInProgress({
  ...
  const copy =
    status === "analyzing"
      ? "Checking every claim against its exact source passage and scope…"
      : `Reading ${sourceHost} and the relevant official pages…`;
  return (
    <div className="research-live" aria-live="polite">
      <div className="research-progress">
        ...
        <div>
          <strong>
            {status === "analyzing"
              ? "Building the reliance map"
              : "Research in progress"}
          </strong>
          <p>{copy}</p>
          <small>Nothing is being sent.</small>
        </div>
      </div>
      {progress && (
        <div className="crawl-progress">
          <div>
            <span>
              {progress.pageCount} official{" "}
              {progress.pageCount === 1 ? "page" : "pages"} opened
            </span>
            <small>
              {progress.total
            ...
            {progress.pages.map((page) => {
              ...
              return (
                <li key={page.url}>
                  <CircleCheck />
                  <span>
                    <strong>{page.title ?? (path || sourceHost)}</strong>
                    <small>{path || "/"}</small>
                  </span>
                </li>
              );
...
function Assessment({
  ...
  const labels: Record<string, string> = {
    established: "Published by provider",
    conditional: "Conditional",
    vague_or_conditional: "Conditional",
  ...
  const icons: Record<string, typeof CircleCheck> = {
    ...
    conditional: CircleDashed,
    vague_or_conditional: CircleDashed,
    conflicting: GitCompareArrows,
    scope_mismatch: AlertTriangle,
    not_established: CircleAlert,
  };
  ...
  return (
    <article className={`assessment assessment-${assessment.status}`}>
      <div className="assessment-status">
        <Icon />
        {labels[assessment.status] ?? assessment.status}
      </div>
      <h3>{assessment.statement}</h3>
      {assessment.assessedScope && (
        <p className="assessment-scope">
          Scope checked: {assessment.assessedScope}
        </p>
      )}
      ...
      {evidence.length > 0 && (
        <div className="evidence-stack">
          {evidence.map((item) => (
            <figure
              key={item._id}
              className={item.supports ? "supports" : "opposes"}
            >
              <figcaption>
                {item.supports ? "Official passage" : "Conflicting passage"}
              </figcaption>
              <blockquote>“{item.sourceExcerpt}”</blockquote>
              <a href={item.sourceUrl} target="_blank" rel="noreferrer">
                {item.sourceTitle ?? "Open official source"}
                <ArrowUpRight />
              </a>
            </figure>
          ))}
...
function SourceRegister({
  ...
  return (
    <details className="source-register">
      <summary>
        Source register <span>{latest.length}</span>
      </summary>
      <ol>
        {latest.map((source) => (
          <li key={source._id}>
            ...
            <small>
              Captured {formatDateTime(source.capturedAt)} · SHA-256{" "}
...
function ConfirmationPanel({ request, contacts }: ConfirmationPanelProps) {
  ...
  const effectiveContactChoice =
    !manualRecipient && contactChoice === "manual" && initialContact
      ? initialContact._id
      : contactChoice;
  ...
  async function save() {
    const selectedContactId =
      effectiveContactChoice === "manual"
        ? undefined
        : (effectiveContactChoice as Id<"officialContacts">);
    setPending("save");
    setError("");
    setNotice("");
    try {
    ...
    <section className="confirmation-panel ticket-shell">
      ...
      {editable ? (
        <>
          ...
          {notice && (
            <p role="status" className="form-notice mb-5">
              {notice}
            </p>
          )}
          <div className="space-y-5">
            <div>
              ...
              {contacts.length > 0 && (
                <Select
                  ...
                  onValueChange={(value) => {
                    ...
                    const contact = contacts.find((item) => item._id === value);
                    setManualRecipient(!contact);
              ...
              {effectiveContactChoice === "manual" ? (
                ...
                <div
                  className="verified-recipient"
                  aria-label={`Source-checked recipient ${selectedContact.email}`}
                >
                  <CircleCheck />
                  <span>
                    <strong>{selectedContact.email}</strong>
                    <small>Published by the provider</small>
                  </span>
                </div>
              ) : null}
              <p className="field-help">
                Published contacts are source-checked. A manual address is
            ...
            <div>
              <Label htmlFor="confirmation-body">Message</Label>
              <Textarea
          ...
          {dirty && (
            <p className="draft-boundary">
              You changed this draft. Save it before approval.
            </p>
          )}
          <label className="approval-check">
            ...
            />
            <span>
              I reviewed this exact recipient, subject, and message. I approve
              sending it now.
            </span>
          </label>
          <Button
            disabled={!approved || dirty || pending !== null}
            className="mt-4 h-12 w-full rounded-none bg-cobalt text-white hover:bg-[#153ae8]"
          ...
          <p className="send-boundary">
            <LockKeyhole /> No automatic outreach. This sends one approved
            email.
          </p>
        </>
      ) : (
        <div className="sent-request">
          <span className="delivery-ring">
            <Send />
          </span>
          <strong>
            {providerFailed
          ...
          <details>
            <summary>View the exact message</summary>
...
function ContactBoundary() {
  return (
    <section className="contact-boundary">
      <Mail />
      <div>
        <strong>No message has been created yet.</strong>
        <p>
          If a consequential gap remains, you’ll review the exact recipient and
          wording here.
        </p>
      </div>
    </section>
...
function Timeline({
  ...
  events: Array<{ _id: string; label: string; occurredAt: number }>;
}) {
  return (
    <section className="case-timeline">
      <p className="ink-label">Case record</p>
      <ol>
        {events
          .slice(-8)
          .reverse()
...
function ReplyPanel({
  ...
  reply: { sender: string; subject: string; body: string; receivedAt: number };
}) {
  return (
    <section className="reply-panel">
      <div className="reply-heading">
        ...
        <div>
          <p className="ink-label">Real reply received</p>
          <h2>{reply.subject || "Written response"}</h2>
          <p>
            From {reply.sender} · {formatDateTime(reply.receivedAt)}
          </p>
        </div>
      </div>
...
function ProofCard({
  ...
  return (
    <section
      className="proof-card ticket-shell"
      aria-labelledby="proof-card-title"
    >
      <div className="proof-card-top">
        <div>
          <p className="ink-label">Private Proof Card</p>
          <h2 id="proof-card-title">
            {card.basis === "official_source"
              ? "Published by provider"
              : proofLabel(card.verdict)}
          </h2>
        </div>
        <span className="proof-seal">
          <ShieldCheck />
          <small>
            {card.basis === "written_reply"
              ? "WRITTEN REPLY"
              : "OFFICIAL SOURCE"}
          </small>
        </span>
      ...
      <div className="proof-items">
        {items.map((item, index) => (
          <article key={item._id}>
            <span>{String(index + 1).padStart(2, "0")}</span>
            <div>
              <p className="proof-item-verdict">
                {card.basis === "official_source"
                  ? "Published by provider"
                  : proofLabel(item.verdict)}
              </p>
              ...
              {item.sourceExcerpts.map((excerpt, excerptIndex) => (
                <blockquote key={`${item._id}-${excerptIndex}`}>
                  “{excerpt}”
                </blockquote>
              ))}
            </div>
      ...
      {card.writtenMessage && (
        <details className="proof-message">
          <summary>Original written reply</summary>
          <p>{card.writtenMessage}</p>
        </details>
      )}
      {card.suggestedFollowUp && (
        <div className="followup-note">
          <strong>One follow-up may resolve what remains</strong>
          <p>{card.suggestedFollowUp}</p>
          {error && (
            <p role="alert" className="form-error">
              {error}
            </p>
          ...
          <Button
            variant="outline"
            disabled={followUpPending || followUpAlreadyPrepared}
            onClick={() => void prepareFollowUp()}
          >
            {followUpPending ? (
              <LoaderCircle className="animate-spin" />
            ) : (
          ...
          <small>
            Nothing is sent until you review and approve the exact message.
          </small>
        </div>
      )}
      <div className="proof-footer">
        <div>
          ...
          <span>
            ...
            <small>
              Original evidence remains preserved even if the source changes.
            </small>
          </span>
        </div>
        <div>
          ...
          {card.receivedAt && (
            <span>
              Received
              <br />
              <strong>{formatDateTime(card.receivedAt)}</strong>
            </span>
          )}
          <Button variant="ghost" disabled={checking} onClick={onCheck}>
            {checking ? (
              <LoaderCircle className="animate-spin" />
            ) : (
              <RefreshCw />
...
function SourceChangeAlert({
  change,
  pending,
  onAcknowledge,
}: {
  change: {
    _id: Id<"sourceChanges">;
    sourceUrl: string;
  ...
  return (
    <section className="source-change-alert" role="alert">
      <div className="source-change-heading">
        <AlertTriangle />
        <div>
          <strong>An official source changed</strong>
          <p>
            Detected {formatDateTime(change.detectedAt)}. Your earlier evidence
            remains intact.
          </p>
        </div>
      </div>
      <div className="source-change-comparison">
        <div>
          <span>Previously preserved</span>
          <blockquote>“{change.previousExcerpt}”</blockquote>
        </div>
        <div>
          <span>Current page</span>
          <blockquote>“{change.currentExcerpt}”</blockquote>
        </div>
      </div>
      <div className="source-change-actions">
        <a href={change.sourceUrl} target="_blank" rel="noreferrer">
          Open current source <ArrowUpRight />
        </a>
        <Button variant="outline" disabled={pending} onClick={onAcknowledge}>
...
function proofLabel(verdict: string) {
  const labels: Record<string, string> = {
    ...
    needs_followup: "Needs one follow-up",
...
function NotFound() {
  return (
    <div className="not-found ticket-shell">
      <LockKeyhole />
      <h1>This private decision is not available.</h1>
      <p>It may belong to a different account or no longer exist.</p>
      <Button asChild>
        <Link to="/app">
          Return to Promise Wallet <ChevronRight />
        </Link>
      </Button>
    </div>
  );
